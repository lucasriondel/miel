...
export function applyCommand(): Command {
  return new Command("apply")
    .description(
      "Apply suggested labels (existing and/or new) from the latest triage of a message",
    )
    .requiredOption(
    ...
    .option(
      "--existing-only",
      "Only accept existing-label suggestions",
      false,
    )
    ...
    .action(
      async (opts: {
        ...
        const pendingNew = latest.newLabelSuggestions.filter(
          (s) => s.status === "pending",
        );

        const includeExisting = opts.newOnly ? false : true;
        const includeNew = opts.existingOnly ? false : true;

        const acceptExistingLabelIds = includeExisting
          ? pendingExisting.map((s) => s.labelId)
          : [];
        const acceptNewSuggestionIds = includeNew
          ? pendingNew.map((s) => s.suggestionId)
          : [];

        if (
          acceptExistingLabelIds.length === 0 &&
          acceptNewSuggestionIds.length === 0
        ) {
