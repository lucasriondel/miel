...
// Run with `MIEL_TEST_ACCOUNT=you@example.com bun scripts/smoke-api.ts`.
...
function requireTestAccount(): string {
  ...
  console.error(
    "MIEL_TEST_ACCOUNT is not set: this script needs the Gmail address to seed against.\n" +
      "  MIEL_TEST_ACCOUNT=you@example.com bun scripts/smoke-api.ts",
  );
  process.exit(1);
}
...
const fakeGog: GogAdapter = {
  ...
  async addAccount(o) {
    ...
    return {
      sessionId: "smoke-add",
      url: "https://accounts.google.com/o/oauth2/auth?state=x",
    ...
    return {
      output: {
        results: input.messages.map((m, i) => ({
          ...
          suggestNewLabels: i === 0 ? [{ name: "SmokeApiNew", reasoning: "stub new label" }] : [],
        })),
      },
...
async function clean() {
  ...
  await db.execute(dsql`DELETE FROM triages WHERE gmail_message_id LIKE 'smoke-api-%'`);
  await db.execute(dsql`DELETE FROM message_labels WHERE gmail_message_id LIKE 'smoke-api-%'`);
  await db.execute(dsql`DELETE FROM messages WHERE gmail_message_id LIKE 'smoke-api-%'`);
...
async function main() {
  ...
  try {
    ...
    const accountRow = accs.body.accounts.find((a: any) => a.email === TEST_ACCOUNT);
    ...
    const msgs = await req("GET", `/messages?account=${accountId}&priority=high&limit=10`);
    console.log(
      "  items =",
    ...
    const detail = await req("GET", `/messages/${accountId}/${msg.gmailMessageId}`);
    console.log(
      "  detail subject =",
      detail.body?.subject,
      "triageHistory =",
    ...
    const existingIds: string[] = detail.body.triageHistory[0].existingLabelSuggestions.map(
      (s: any) => s.labelId,
    );
    const newIds: string[] = detail.body.triageHistory[0].newLabelSuggestions.map(
    ...
    const msg2 = msgs.body.items.find((m: any) => m.gmailMessageId.endsWith("-2"));
    if (msg2) {
      const arch = await req("POST", `/messages/${accountId}/${msg2.gmailMessageId}/archive`);
      console.log("  archive =", arch.body);

      console.log("> DELETE /messages/:id2 (trash)");
      const tr = await req("DELETE", `/messages/${accountId}/${msg2.gmailMessageId}`);
    ...
    const nf = await req("GET", "/accounts/00000000-0000-0000-0000-000000000000/labels");
