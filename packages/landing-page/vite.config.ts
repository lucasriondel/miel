...
export default defineConfig({
  ...
  //
  // Behind portless (`portless.json` at the root) the port is ephemeral and
  // arrives as `PORT`, and `landing.miel.localhost` is the stable address
  // instead; run directly (`PORTLESS=0 bun dev:app`) the registry's row is both.
  // Taking `PORT` first serves either spelling off one config, and `strictPort`
  // still holds for the pinned case.
  server: { port: Number(process.env.PORT ?? 5200), strictPort: true },
