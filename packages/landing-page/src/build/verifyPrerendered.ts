...
import { LUCIDE_LICENSE } from "../content/icons";
...
const HOME_CONTRACT: PageContract = {
  ...
  text: unique([
    ...
    // The icon licence's notice, which has to be rendered rather than merely
    // shipped — see `content/icons.ts`.
    LUCIDE_LICENSE,
  ...
  markup: [
    ...
    // "What is Miel" heads one section rather than five, so its parts are h3s
    // inside it — the level is what says they are parts of that answer.
    `<h2 id="${ABOUT.id}-heading">${escapeHtml(ABOUT.heading)}</h2>`,
    `<h3 id="${FEATURES.id}-heading">`,
    // The side menu links to "What is Miel" and to every guide section, so a
    // reader who arrives at the foot of the page still has a way into each one.
    ...[ABOUT.id, ...GUIDE_SECTIONS.map((section) => section.id)].map((id) => `href="#${id}"`),
  ],
  links: [GITHUB_URL, `mailto:${CONTACT_EMAIL}`, PRIVACY.path, TERMS.path],
  absentLinks: [HOME_PATH],
  counts: [
    ...DOCUMENT_COUNTS,
    // "What is Miel" and the subsections nested in it (the feature list plus
    // the prose blocks), the guide sections the side menu navigates, plus
    // permissions, the AI-provider disclosure and contact.
    {
      what: "section",
      pattern: /<section[\s>]/g,
      expected: 1 + 1 + HOME.sections.length + GUIDE_SECTIONS.length + 3,
    },
    // One name/body pair per feature, as a description list rather than a grid
    // of divs — so the pairing survives CSS being off.
    { what: "feature term", pattern: /<dt>/g, expected: FEATURES.items.length },
    // Each glyph is an inline <svg>, not an <img> or a background image: the
    // page must fetch nothing, and findExternalReferences reports both of those.
    {
      what: "feature glyph",
      pattern: /<svg[^>]*class="feature-icon"/g,
      expected: FEATURES.items.length,
    },
    { what: "feature description", pattern: /<dd>/g, expected: FEATURES.items.length },
