 ...
 * owns port 5200 from the shared registry when it is run directly and fails
 * loudly on a clash — behind portless it takes the ephemeral `PORT` instead and
 * answers at `landing.miel.localhost` — and the package installs for anyone who
 * clones the repo — no authenticated registry.
 */
...
describe("dev server port", () => {
  ...
  /**
   * The pin lives in the vite config alone, not in the dev script too. Behind
   * portless the port is ephemeral and arrives as `PORT`, and a `--port` flag on
   * the command line would override it and defeat the proxy — so `dev:app` is
   * the bare `vite dev` and the config decides, taking `PORT` when there is one
   * and 5200 when there is not.
   */
  test("is left to the config, so portless's injected PORT is not overridden", () => {
    expect(packageJson.scripts["dev:app"]).toBe("vite dev");
    expect(packageJson.scripts["dev:app"]).not.toContain("--port");
  });

  test("is reached through portless by the plain `dev` script", () => {
    expect(packageJson.scripts.dev).toBe("portless");
  });
