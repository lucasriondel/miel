...
// The tasks that send a whole message body (#158), so the pointer this page
// makes at the disclosure is checked against what the disclosure says.
...
import { CLAUDE_DISCLOSURE } from "./site";
...
describe("motivation", () => {
  ...
  /* Issue #114: the sentence that admits mail leaves the machine has to offer
     ...
     unmentioned. */
  /* Issue #158. Two promises are made in this one sentence and both had gone
     stale when a second AI task started sending mail out: that what leaves is
     what goes to the provider — triage is no longer the only sender — and that
     the detail is "spelled out further down this page", which is the homepage
     disclosure, further down the same page. So the pointer is checked against
     what it points at, the way the disclosure's pointer here already is. */
  describe("the pointer to what each provider is sent", () => {
    test("does not pin what leaves the machine on triage alone", () => {
      const sentence = text
        .split(/(?<=\.)\s+/)
        .find((candidate) => /leaves your machine/i.test(candidate));
      expect(sentence).not.toMatch(/provider running your triage/i);
    });

    test("promises the detail is further down the page", () => {
      expect(text).toMatch(/spelled out further down this page/i);
    });

    test("the disclosure it points at names every task that sends a body", () => {
      const disclosure = CLAUDE_DISCLOSURE.body.join("\n").toLowerCase();
      for (const task of BODY_BEARING_TASKS) {
        expect(disclosure).toContain(task.label.toLowerCase());
      }
    });
  });
...
describe("installation", () => {
  ...
  describe("the provider a fresh install already runs on", () => {
    ...
    /* Issue #154. How many AI tasks there are is the catalogue's answer and it
       grows — promo extraction was the fourth, and it runs on every sync. Copy
       that counts them, or lists them by name, dates itself on the next one and
       reads to a reader as a promise about where the task it forgot runs. The
       settings panel is the list; the guide says "every task". */
    test("does not count the tasks a provider pick covers", () => {
      expect(text).not.toMatch(/all (three|four) tasks/i);
      expect(text).not.toMatch(/triage, repl(y|ies) and filter/i);
    });
  });
});
