...
describe("privacy policy: transfer to the AI provider", () => {
  ...
  /* Issue #158. A reader of this page has to come away knowing which requests
     carry a body — since #156 that is two, not one. The list and the words for
     it are core's (`claudeUsage.ts`), so the page quotes them and a third task
     fails here until this section names it. */
  describe("the requests that carry a whole message body", () => {
    // Case-insensitively on the label: it is written to sit mid-sentence and
    // the page starts a sentence with it (`sentenceCase`).
    test.each([...BODY_BEARING_TASKS])("names $task, and what of the message it sends", (task) => {
      expect(text.toLowerCase()).toContain(task.label.toLowerCase());
      expect(text).toContain(task.sends);
    });

    test("says how many of them there are", () => {
      expect(text).toMatch(new RegExp(`\\b${amount(BODY_BEARING_TASKS.length)}\\b`, "i"));
    });

    test("no longer says reply drafting is the only one", () => {
    ...
    test("still says triage is not one of them", () => {
      expect(text).toMatch(/full bodies are not part of that request/i);
    });
  });
});
