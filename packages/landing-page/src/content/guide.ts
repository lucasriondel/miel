...
export const INSTALLATION: GuideSection = {
  ...
  body: [
    ...
    "You will need a credential for one AI provider, and one is already picked for you: claude-code, the local Claude Code CLI, is the default, and a fresh install runs every one of its AI tasks through it. So the last prerequisite is that CLI on your PATH with a token from claude setup-token. Change the provider in settings to Anthropic, Google or OpenAI and the prerequisite becomes an API key from that vendor instead, with no CLI at all.",
  ...
  steps: [
    ...
    {
      label: "Run the app",
      /* `bun start` rather than `bun dev`: this is a self-hosting guide, and
         `bun dev` is the development stack — file watching, hot reload, the
         dev server. `bun start` builds first and serves the built bundle. */
      body: "This builds the API and the web app, then serves them. The web app comes up on port 5230 and the API on port 5531. Open http://localhost:5230/app and click Connect with Google. Use bun dev instead if you are working on miel rather than running it.",
      code: "bun start",
      variant: "bun",
    },
