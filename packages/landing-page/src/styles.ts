...
/* The parts of "What is Miel" are sections for the accessible name and the
   anchor, not for the rule across the top: one heading owns that block, and a
   line under each of its five parts would read as five blocks again. */
.subsection { border-top: none; padding-top: 0; }
...
/* The first subsection follows its own h2, which already carries the space. */
.subsection:first-of-type > h3 { margin-top: 1rem; }
...
/* The third-party notice the icon licence asks for: present and readable, but
   set below the footer's own text so it does not compete with it. */
.colophon {
  color: var(--muted);
  font-size: 0.78rem;
  line-height: 1.45;
...
/* Ruled rows rather than a grid of cards. Two things follow from that, and
   both are the reason for it: the number of features stops affecting the
   layout — an eighth is another row, where a two-column grid left the seventh
   alone on a row of its own — and a short description no longer sits in a box
   stretched to the height of a long one beside it. The rule is the same
   --rule the permissions table below uses, so the two read as one document. */
.feature-list {
  margin: 1.25rem 0 1rem;
  border-top: 1px solid var(--rule);
}

/* Three columns: the glyph, the name, the description. The name column is
   bounded rather than fluid so every description starts on the same axis down
   the list, and it is a max rather than a fixed width so a long name wraps
   inside its own column instead of pushing the description narrower. */
.feature {
  display: grid;
  grid-template-columns: 1.6rem minmax(0, 15rem) minmax(0, 1fr);
  gap: 0 1rem;
  align-items: start;
  padding: 0.95rem 0;
  border-bottom: 1px solid var(--rule);
...
/* dd carries a default indent in every browser; the columns are the alignment
   here, so it is removed rather than added to. */
.feature dd {
  margin: 0;
  color: var(--muted);
  font-size: 0.95rem;
...
/* Sized in rem rather than em: it is its own column now, not part of the
   heading it used to sit inside, so it should not resize with the name beside
   it. The top margin is optical — a 24x24 square hung from the grid row's top
   edge sits high against the first line of a 1.5 line-height name. */
.feature-icon {
  width: 1.15rem;
  height: 1.15rem;
  flex: none;
...
/* Below the point where a name and its description stop both fitting, the two
   stack under the glyph rather than the columns squeezing. The description
   moves into the name's column so it stays aligned with it, indented past the
   glyph that labels the pair. */
...
/* Screenshot on top, copy under it, one column at every width. The shot is a
   1800x1209 landscape image and the whole point of the page's first screen, so
   it takes the full column rather than half of it — at half width the message
   rows inside it stop being legible, which is the thing a reader came to see.
...
/* The copy is a reading column, not the full width of the image above it, and
   it is centred under the shot so the two share an axis. */
.hero-copy {
  min-width: 0;
  max-width: 44rem;
  margin-inline: auto;
...
/* Centred copy stops paying its way once the lines are short enough to wrap
   three or four times; below the page's one breakpoint it reads left. */
@media (max-width: 52rem) {
  .hero { gap: 2rem; }
...
/* The image is a data URI in a style attribute (see AppPreview.tsx); what is
   ...
   It faces the reader square on. The frame used to be turned away — a
   perspective rotation about Y, the way a slide looks seen from its side —
   which was worth its cost while the shot sat in a half-width track beside the
   copy. At the head of the page and at full width it is the thing being read,
   and a turn away from the reader trades exactly the legibility that placement
   is buying. tools/transform-tuner.html is still there if it ever turns again. */
.shot {
  position: relative;
