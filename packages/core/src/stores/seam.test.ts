...
const SEAMED_SERVICES = [
  ...
  "../services/apply.ts",
  "../services/labels.ts",
  // The promo aggregate joined the list when its first read got a Promise
  // facade (#160); the extraction beside it runs inside a sync, which provides
  // the adapter itself.
...
describe("both adapters are held to one contract", () => {
  ...
  test.each(APPLIES_CONTRACT)("%s runs the shared spec", async (path) => {
    ...
    // The promo aggregate (#157) — the sixth store, and the first whose rows
    // hang off a message, so each side supplies the mail its own way.
    expect(source).toContain("promoStoreContract(");
  });
