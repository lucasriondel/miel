...
const EnvSchema = z.object({
  ...
  // The registry's rows for miel (~/dev/PORTS.md), not framework defaults: the
  // old 3001/3000 sat in the reserved 3000-3099 band the registry keeps free so
  // a stray default never collides with a registered app. Moving one means
  // moving its callers — the web dev proxy, WEB_ORIGIN below and
  // DEV_GOOGLE_REDIRECT_URI, which derives from API_PORT.
  API_PORT: z.coerce.number().int().positive().default(5531),
  WEB_PORT: z.coerce.number().int().positive().default(5230),
  // A single bare scheme://host, deliberately: it is the base the OAuth callback
  // redirects the browser back to as well as the CORS origin, and a list would
  // make an unusable redirect. The second dev origin the SPA is reachable by —
  // the portless hostname — is added to the CORS allowlist in `createApp`, which
  // is the half of the job that can take more than one.
  WEB_ORIGIN: z.string().default("http://localhost:5230"),
