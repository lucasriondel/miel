...
export interface ReconcileDiffInput {
  /**
   * All messages currently in DB whose internalDate falls in the window the
   * search actually covered — not the window the caller asked for. Anything
   * outside it was never searched, so its absence from `hits` means nothing.
   */
...
export interface ReconcileStepArgs {
  ...
  accountEmail: string;
  rangeFrom: Date;
  /** Exclusive, and the end of the *searched* window — `resolveSyncRange`'s `to`. */
  rangeTo: Date;
  hits: Set<string>;
  capReached: boolean;
  log: (msg: string) => void;
      ...
      db
        ...
        .where(
          and(
            ...
            // Exclusive: `rangeTo` is where the search stopped, not the last
            // instant it covered, so a message landing exactly on it was never
            // searched for and must stay out of scope.
            lt(messages.internalDate, args.rangeTo),
          ),
        ),
    );
