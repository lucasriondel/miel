...
describe("the provider a fresh install starts on", () => {
  // Swept off MODEL_TASKS rather than a list of its own, so the fourth task
  // (#156) and any after it are covered by being tasks at all.
  test.each([...MODEL_TASKS])("%s defaults to DEFAULT_PROVIDER", (task) => {
    expect(SETTING_DEFAULTS[SETTING_KEYS[`${task}Provider`]]).toBe(DEFAULT_PROVIDER);
    expect(DEFAULTS[`${task}Provider`]).toBe(DEFAULT_PROVIDER);
  });
  ...
  test("every task's default model is one that provider serves", () => {
    for (const task of MODEL_TASKS) {
      expect(isModelForProvider(DEFAULT_PROVIDER, DEFAULTS[`${task}Model`])).toBe(true);
    }
  });
});
