...
import { applyLabelsEffect, applySuggestionsEffect, dismissSuggestionsEffect } from "./apply";
...
// "Dismiss all" is the other half of the row's suggestion pill: the user turning
// the whole set down. Unlike accepting, it reaches nothing outside Postgres —
// declining changes what miel offers, never what Gmail holds — so what is worth
// asserting is which rows move and that Gmail is left alone.
describe("dismissSuggestionsEffect", () => {
  beforeEach(() => {
    stores.mailbox.add.triage({
      id: "t1",
      accountId: ACCOUNT,
  ...
  test("turns down both kinds of pending suggestion at once", async () => {
    const result = await stores.run(
    ...
    expect(result).toEqual({ ok: true, triageId: "t1" });
    expect(stores.mailbox.existingSuggestions[0].status).toBe("dismissed");
    expect(stores.mailbox.newSuggestions[0].status).toBe("dismissed");
  });

  test("attaches nothing and tells Gmail nothing", async () => {
    await stores.run(
      dismissSuggestionsEffect({
        triageId: "t1",
        accountId: ACCOUNT,
  ...
  test("leaves an already-applied suggestion applied", async () => {
    // It is a label the message now carries; calling it dismissed would claim
    // otherwise.
    await stores.run(
      applySuggestionsEffect({
        triageId: "t1",
  ...
  test("refuses a triage that belongs to another message", async () => {
    const exit = await runExit(
      stores.provide(
        dismissSuggestionsEffect({
          triageId: "t1",
  ...
  test("refuses a triage that does not exist", async () => {
    const exit = await runExit(
      stores.provide(
        dismissSuggestionsEffect({
          triageId: "nope",
