...
import { LabelStore, MessageStore, PromoStore, TriageStore } from "../stores/contracts";
...
// The promo store's behaviour is the shared contract's (run in `stores.test.ts`);
// what is this adapter's own is the seeding, and the rows as they stand after.
describe("the in-memory promo store", () => {
  test("can be seeded with a promo a test wants to find already extracted", async () => {
    const seeded = makeTestStores({
      mailbox: {
        accounts: [{ id: ACCOUNT, email: "me@example.com" }],
        messages: [{ accountId: ACCOUNT, gmailMessageId: "m1", internalDate: "2026-03-01" }],
        promos: [{ id: "promo-1", ...REF, discount: "20% off", code: "SUMMER20" }],
      },
    });

    const suggestions = await seeded.run(PromoStore.unsaved({ accountId: ACCOUNT }));

    expect(suggestions.map((p) => p.code)).toEqual(["SUMMER20"]);
    // A seed carries no copy of the mail: that is written by a save, and this
    // one has not been saved.
    expect(suggestions[0]).toMatchObject({ savedAt: null, subject: null, bodyHtml: null });
  });

  test("leaves the rows readable after a write, for suites that assert what is stored", async () => {
    await stores.run(
      PromoStore.insert([
    ...
    expect(stores.mailbox.promos).toHaveLength(1);
  });
});
...
describe("an offline mailbox", () => {
  test("fails a promo read rather than answering no suggestions", async () => {
    stores.mailbox.offline = true;

    const exit = await Effect.runPromise(
      Effect.exit(stores.provide(PromoStore.unsaved({ accountId: ACCOUNT }))),
    );
