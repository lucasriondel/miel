...
export interface TestStores {
  ...
  /**
   * The mailbox behind `MessageStore`, `TriageStore`, `LabelStore` and
   * `PromoStore` — seeded rows, and the rows as they stand after the service
   * under test wrote.
   */
...
export function makeTestStores(seed?: {
  ...
  const layer = Layer.mergeAll(
    ...
    Layer.succeed(PromoStore, mailbox.promoStore),
  );
