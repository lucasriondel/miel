...
import { promoStoreContract, secretStoreContract, settingsStoreContract } from "./storeContract";
...
// The promo aggregate (#157) hangs off the mailbox, so its world is the fake
// mailbox: an account and a message are rows a test adds, not a table it
// creates.
promoStoreContract("the in-memory promo store", async () => {
  const fresh = makeTestStores();
  let n = 0;
  return {
    layer: fresh.layer,
    account: async () => {
      ...
      fresh.mailbox.add.account({ id, email });
      return { id, email };
    },
    message: async ({ accountId, internalDate, isTrashed }) => {
      const gmailMessageId = `m${++n}`;
      fresh.mailbox.add.message({ accountId, gmailMessageId, internalDate, isTrashed });
