...
/**
 * The AI tasks that each carry their own provider + model. Iterated rather than
 * spelled out once per task: the settings service builds its keys and its
 * `ModelSettings` fields from it, so one list keeps them from drifting.
 *
 ...
 * to move with it.
 *
 * A task's name is also a key prefix — `${task}.provider` in `app_settings`,
 * `${task}Provider` on `ModelSettings` — so it stays lower-case and hyphenated
 * (`promo-extract`, #156) rather than gaining a case or a separator those two
 * spellings cannot carry.
 */
export const MODEL_TASKS = ["triage", "reply", "filter", "promo-extract"] as const;
