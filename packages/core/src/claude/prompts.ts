...
export function buildPromoExtractPrompt(input: PromoExtractInputT): string {
  ...
  return [
    ...
    "- code: the redemption code exactly as written, case and all. Every entry must carry one.",
    "- Do not return an offer that needs no code (free shipping applied automatically at checkout, a sitewide sale price). Skip it and carry on with the rest of the mail.",
    ...
    "- Return an empty promos array when there is nothing to return — no offer at all, or only offers needing no code. That is an answer, not a failure.",
    "",
    "Email text:",
    body,
  ].join("\n");
