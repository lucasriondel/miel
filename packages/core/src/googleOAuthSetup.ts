 ...
 *
 * **This is why the API is the one package pinned to a port under portless.**
 * Portless (`portless.json`) otherwise assigns each dev server an ephemeral
 * port and fronts it at a name; this package is given `appPort: 5531` instead.
 * Two constraints leave nothing else: Google matches a redirect URI **byte for
 * byte**, so it cannot contain a port that changes at every boot — and Google
 * **rejects `.localhost` URIs**, so `https://api.miel.localhost` cannot be
 * registered in its place. A fixed loopback port is the only address that
 * satisfies both, so this stays literal and portless is told to honour it.
 */
export const DEV_GOOGLE_REDIRECT_URI = `http://localhost:5531${GOOGLE_OAUTH_CALLBACK_PATH}`;
