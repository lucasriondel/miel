...
import type { ListMessagesResponse, ListedMessage, MessageDetail } from "./types";
...
type DetailSnapshot = Array<[readonly unknown[], MessageDetail | undefined]>;
...
/**
 * What the mutation expects the open message to become (#145).
 *
 * The detail is a different shape from a list row — a priority lives on its
 * newest triage run, a suggestion is a status on that run rather than an entry
 * in `pendingSuggestions` — so it is a plan of its own rather than the list's
 * applied twice. It is only ever asked about a message that is already cached:
 * an action taken from a row must not seed a detail nobody has opened.
 */
export type OptimisticDetail<TInput> = (input: TInput) => (detail: MessageDetail) => MessageDetail;

/**
 * A second `{ items }` cache the mutation moves with the message lists (#161).
 *
 * Saving a promo takes a card out of the suggestions and the mail out of the
 * list in one press, and the two live under different query keys on purpose —
 * different lifetimes, different invalidation. What they must share is this
 * choreography: cancelled, snapshotted, written and rolled back in the same
 * step, so a refused save cannot put one back without the other.
 *
 * A whole plan rather than a list of ids, because what leaves is the mutation's
 * rule to state — and `null` drops an item, exactly as it does for a row.
 */
export interface OptimisticSideList<TInput, TItem> {
  /** The key prefix whose cached `{ items }` move with the lists. */
  key: readonly unknown[];
  /** What each cached item becomes; `null` drops it. */
  item: (input: TInput) => (item: TItem) => TItem | null;
}

/** A side cache as this module reads one: a payload with items in it. */
type SideCacheValue<TItem> = { items: TItem[] };

/**
 * Kept opaque so the context stays one type for every mutation: putting a
 * snapshot back is `setQueryData(key, data)` whatever the data was.
 */
type SideSnapshot = Array<[readonly unknown[], unknown]>;

export interface MessageMutationConfig<TInput extends MessageMutationTarget, TSide = never> {
  ...
  /**
   * How the message-detail view should read before the server answers. Omit for
   * a change the page the user stays on cannot show.
   */
  optimisticDetail?: OptimisticDetail<TInput>;
  /**
   * Another list-shaped query written and rolled back in the same step as the
   * lists. Re-reading it is `alsoInvalidate`'s half, as it is for any other key.
   */
  optimisticSide?: OptimisticSideList<TInput, TSide>;
...
export function messageMutationOptions<
  TInput extends MessageMutationTarget,
  TResult,
  TSide = never,
>(qc: QueryClient, config: MessageMutationConfig<TInput, TSide>) {
  ...
  return {
    ...
    onMutate: async (input: TInput): Promise<MessageMutationContext> => {
      const detailKeys = messageDetailKeys(input);
      const sideKey = config.optimisticSide?.key;
      await Promise.all([
        qc.cancelQueries({ queryKey: MESSAGES_KEY }),
        ...detailKeys.map((queryKey) => qc.cancelQueries({ queryKey })),
        ...(sideKey ? [qc.cancelQueries({ queryKey: sideKey })] : []),
      ]);

      const snapshot = qc.getQueriesData<MessagesCacheValue>({ queryKey: MESSAGES_KEY });
      const detail: DetailSnapshot = detailKeys.map((key) => [
        key,
        qc.getQueryData<MessageDetail>(key),
      ]);
      ...
      const detailPlan = config.optimisticDetail?.(input);
      if (detailPlan) {
        for (const [key] of detail) {
          qc.setQueryData<MessageDetail>(key, (data) =>
            data === undefined ? data : detailPlan(data),
          );
        }
      }

      const side: SideSnapshot = sideKey
        ? qc.getQueriesData<SideCacheValue<TSide>>({ queryKey: sideKey })
        : [];
      const sidePlan = config.optimisticSide?.item(input);
      if (sideKey && sidePlan) {
        qc.setQueriesData<SideCacheValue<TSide>>({ queryKey: sideKey }, (data) =>
          data === undefined
            ? data
            : { ...data, items: data.items.map(sidePlan).filter((i): i is TSide => i !== null) },
        );
      }

      return { snapshot, detail, side };
    },
    ...
    onError: (_err: unknown, input: TInput, context?: MessageMutationContext) => {
      ...
      // The detail goes back too, and before the invalidation below: the user
      // may still be reading the page the action was taken on, and a refetch is
      // no answer to a badge that should never have moved.
      for (const [key, data] of context?.detail ?? []) {
        qc.setQueryData(key, data);
      }
      // …and so does the side cache, in the same step: the card and the row
      // left together, so they have to come back together.
      for (const [key, data] of context?.side ?? []) {
        qc.setQueryData(key, data);
      }
      qc.invalidateQueries({ queryKey: MESSAGES_KEY });
      invalidateTargets(input);
