...
/**
 * What the app may ask a sync for. The wire protocol also takes `since`, a
 * preset window like "7d" — the scheduler and the CLI still speak it — but no
 * browser gesture does: a sync fetches the period the inbox is showing (#152),
 * so every caller here names an explicit range and the field is gone rather
 * than merely unused (#153), which makes a second period source a type error.
 */
