...
import { messageDetail } from "./messageDetail.fixture";
...
// #145. The lists were the only cache written before the answer came back, so
// an action taken from the detail page — a label removed, a priority changed —
// showed nothing until a refetch landed, on the one screen the user is still
// looking at.
describe("the message the user has open", () => {
  const detailSubject = (qc: QueryClient) =>
    qc.getQueryData<MessageDetail>(detailKey)?.subject ?? null;

  const withDetail = (qc: QueryClient) => {
    qc.setQueryData(detailKey, messageDetail({ subject: "one" }));
    return qc;
  };

  test("is written optimistically, not merely invalidated", async () => {
    const qc = withDetail(seed(new QueryClient()));

    await optionsFor(qc, {
      request: () => ({ path: "/noop", method: "POST" }),
      optimisticDetail: () => (d) => ({ ...d, subject: "edited" }),
    }).onMutate(INPUT);

    expect(detailSubject(qc)).toBe("edited");
  });

  test("rolls back with the lists when the request failed", async () => {
    const qc = withDetail(seed(new QueryClient()));
    ...
    expect(detailSubject(qc)).toBe("one");
    expect(plainSubjects(qc)).toEqual(["one", "two"]);
  });

  test("is left alone by a mutation that declares no detail write", async () => {
    const qc = withDetail(seed(new QueryClient()));
  ...
  // Nothing is invented for a message that was never opened: writing one here
  // would seed the detail cache from a list row, and the page would then render
  // a message with no body until the real read landed.
  test("is not created by a mutation acting on a message nobody opened", async () => {
    const qc = seed(new QueryClient());
