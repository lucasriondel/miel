...
/**
 * Saves the scroll offset of `el` against the history entry being left, and
 * restores it on the way back. Takes the element rather than a ref object: the
 * layout gets its scroll region through a callback ref routed into state (the
 * top bar has to re-render once there is something to watch), and an effect
 * keyed on the node re-binds if the region is ever remounted.
 *
