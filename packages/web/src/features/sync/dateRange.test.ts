...
describe("startOfPeriod", () => {
  ...
  test("year snaps back to January 1st", () => {
    expect(startOfPeriod(new Date(2026, 5, 17), "year")).toEqual(new Date(2026, 0, 1));
    expect(startOfPeriod(new Date(2026, 0, 1), "year")).toEqual(new Date(2026, 0, 1));
    expect(startOfPeriod(new Date(2026, 11, 31), "year")).toEqual(new Date(2026, 0, 1));
  });

  test("drops the time of day", () => {
    expect(startOfPeriod(new Date(2026, 5, 17, 23, 45, 12), "month")).toEqual(new Date(2026, 5, 1));
    expect(startOfPeriod(new Date(2026, 5, 17, 23, 45, 12), "year")).toEqual(new Date(2026, 0, 1));
  });
});
...
describe("buildRange", () => {
  ...
  test("year ends on next January 1st (exclusive)", () => {
    const range = buildRange(new Date(2026, 5, 3), "year");
    expect(range.start).toEqual(new Date(2026, 0, 1));
    expect(range.end).toEqual(new Date(2027, 0, 1));
    expect(range.key).toBe("2026-01-01");
    expect(range.mode).toBe("year");
  });

  test("a leap year is a whole year, not 365 days", () => {
    const range = buildRange(new Date(2024, 1, 29), "year");
    expect(range.start).toEqual(new Date(2024, 0, 1));
    expect(range.end).toEqual(new Date(2025, 0, 1));
  });
...
describe("stepRange", () => {
  ...
  test("year steps by one calendar year in both directions", () => {
    expect(stepRange(new Date(2026, 0, 1), "year", -1)).toEqual(new Date(2025, 0, 1));
    expect(stepRange(new Date(2026, 0, 1), "year", 1)).toEqual(new Date(2027, 0, 1));
  });

  test("year steps across a leap year exactly, not by 365 days", () => {
    // 2024 has 366 days; both steps still land on January 1st.
    expect(stepRange(new Date(2024, 0, 1), "year", 1)).toEqual(new Date(2025, 0, 1));
    expect(stepRange(new Date(2025, 0, 1), "year", -1)).toEqual(new Date(2024, 0, 1));
  });
});
...
describe("parseRangeKey", () => {
  ...
  test("a year round-trips through its key", () => {
    const range = buildRange(new Date(2026, 5, 17), "year");
    expect(parseRangeKey(range.key, "year")).toEqual(range.start);
    // A key from another period's grid still names the year it falls in.
    expect(parseRangeKey("2026-06-17", "year")).toEqual(new Date(2026, 0, 1));
  });
...
describe("formatRangeLabel", () => {
  ...
  test("year renders as the four-digit year alone", () => {
    expect(label(new Date(2026, 5, 17), "year")).toBe("2026");
    expect(label(new Date(2024, 0, 1), "year")).toBe("2024");
  });
});
