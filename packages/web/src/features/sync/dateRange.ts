...
/**
 * Snaps `d` back to the start of its week (Monday), month (the 1st) or year
 * (January 1st), at 00:00 local.
 */
...
/**
 * Moves `start` by `n` whole periods. Month and year steps are calendar ones —
 * not 30 or 365 days — so stepping across a leap year stays exact.
 */
...
/** The day a period's `end` excludes — the last one it actually covers. */
...
/** Human label for the pager: "Jun 15 – Jun 21, 2026", "June 2026" or "2026". */
export function formatRangeLabel(range: DateRange): string {
  if (range.mode === "year") return YEAR_FMT.format(range.start);
...
/**
 * The three periods as bare nouns, in the order the Sync button stacks them to
 * size itself. It is a tuple over `RangeMode`, not a free list of strings, so a
 * fourth period cannot be added to the pager without a word to name it here.
 */
export const RANGE_MODE_NOUNS: readonly RangeMode[] = ["week", "month", "year"];
