...
export const MessageDetailHeader = ({ message }: Props) => {
  ...
  // `MessageLabels` self-hides when empty, so the wrapping row has to ask the
  // same question it does — otherwise an empty flex row still spends the
  // header's gap. Attachments used to count here; since #143 they are a section
  // of their own below the body, so a message whose only badge was one leaves
  // no row behind.
  const hasBadges =
