...
describe("the badge row", () => {
  test("renders labels, still interactive", () => {
    const html = render();
    expect(html).toContain("Work");
    expect(html).toContain('aria-label="Remove label Work"');
  });

  // #143: an attachment arrived with the message, it is not metadata about it,
  // so it is no longer a badge sharing this row — it has its own section under
  // the body (`features/message-detail/MessageAttachmentsSection.tsx`).
  test("carries no attachment", () => {
    const html = render();
    expect(html).not.toContain("quote.pdf");
