...
// segmented control. What is asserted here is the markup the block decides at
// first render — the classes DESIGN.md legislates (§1 active fill, §2 depth,
// §3 shape, §4 section label, §5 press) plus which of the three body surfaces
// each kind of message lands on. What the block *does* — switching modes,
// obeying the remote-images preference — is rendered next door
// (`remoteImagesPreference.test.tsx`); the iframe's post-load resizing is
// browser behaviour and stays pinned by the source assertion at the bottom.
//
// Since #149 the images toggle exists only under the hide-by-default
// preference, so the block below asks for it before asserting how it looks.
...
/** The preference the per-message toggle belongs to. */
const hidingRemoteImages = () => writeRemoteImagesPreference("hide");
...
describe("the iframe's auto-sizing", () => {
  ...
  test("renders the frame sandboxed, same-origin kept for the measuring", () => {
    // Which flags that sandbox names is `HtmlBodyFrame.test.tsx`'s subject;
    // what matters here is that the block hands the body to a sandboxed frame
    // at all, and that the flag the resize logic depends on is among them.
    expect(render(withHtml)).toMatch(/<iframe[^>]*\ssandbox="[^"]*allow-same-origin/);
  });
});
