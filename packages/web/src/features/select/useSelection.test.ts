...
describe("deselectMany", () => {
  test("removes every id under the given account, leaving others intact", () => {
    const seeded: SelectionState = {
      selectMode: true,
      selectedIds: new Set(["a1|m1", "a1|m2", "a1|m3", "a2|m1"]),
    };
    const next = deselectMany(seeded, "a1", ["m1", "m2"]);
    expect(selectionForAccount(next, "a1")).toEqual(["m3"]);
    // Same id, other account: the key is what is removed, never the id.
    expect(selectionForAccount(next, "a2")).toEqual(["m1"]);
  });
});
...
describe("areAllSelected", () => {
  const seeded: SelectionState = {
    selectMode: true,
    selectedIds: new Set(["a1|m1", "a1|m2", "a2|m3"]),
  };

  test("true only when every id of that account is selected", () => {
    expect(areAllSelected(seeded, "a1", ["m1", "m2"])).toBe(true);
    expect(areAllSelected(seeded, "a1", ["m1", "m2", "m3"])).toBe(false);
    // m3 is selected, but under another account.
    expect(areAllSelected(seeded, "a1", ["m3"])).toBe(false);
  });

  test("an empty category is not a fully selected one", () => {
    expect(areAllSelected(seeded, "a1", [])).toBe(false);
  });
});

describe("toggleManySelection", () => {
  test("selects the whole list when part of it is missing", () => {
    const seeded: SelectionState = {
      selectMode: true,
      selectedIds: new Set(["a1|m1"]),
    };
    const next = toggleManySelection(seeded, "a1", ["m1", "m2"]);
    expect(selectionForAccount(next, "a1").toSorted()).toEqual(["m1", "m2"]);
  });

  test("clears the list when it is already whole, and nothing beside it", () => {
    const seeded: SelectionState = {
      selectMode: true,
      selectedIds: new Set(["a1|m1", "a1|m2", "a1|other"]),
    };
    const next = toggleManySelection(seeded, "a1", ["m1", "m2"]);
    expect(selectionForAccount(next, "a1")).toEqual(["other"]);
  });
});
