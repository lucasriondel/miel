...
// What the published disclosure says goes to a provider (#158): the tasks that
// send a whole message body, and the truncation every one of them applies.
import { BODY_BEARING_TASKS, REPLY_BODY_TRUNCATION } from "@miel/core/claudeUsage";
...
describe("CONTRIBUTING.md", () => {
  ...
  test("gets the reader from a clone to a running stack", () => {
    ...
    // The address `bun dev` actually serves the app at: portless
    // (`portless.json`) fronts the web dev server at this hostname, and the port
    // it binds behind the proxy is ephemeral, so the hostname is the only stable
    // thing to point a reader at.
    expect(contributing).toContain("https://miel.localhost/app");
  });
...
describe("SECURITY.md", () => {
  ...
  /* Issue #158. The scope list names what leaves the machine and sends a
     reporter to `claudeUsage.ts` for the detail, so it is disclosure copy like
     the other three: it may not enumerate the tasks itself — it had two of what
     are now more — and it may not name a vendor as the recipient, which is the
     user's own per-task pick. */
  test("scopes what leaves the machine by pointing at the module, not by listing tasks", () => {
    const bullet = security.split("\n").find((line) => /leaves the machine/i.test(line));
    expect(bullet).toBeDefined();
    expect(bullet).toContain("packages/core/src/claudeUsage.ts");
    expect(bullet).not.toMatch(/\b(Anthropic|OpenAI|Google)\b/);
  });
});
...
describe("what a fresh install triages with", () => {
  ...
  /* Issue #154. Every AI task carries its own provider pick, and the number of
     them grows — promo extraction was the fourth. Copy that lists the tasks by
     name, or counts them, is copy that goes stale on the next one and reads as
     a promise that the task it forgot runs somewhere else. The panel lists
     them; the documents say "every task". The README's own feature description
     is exempt: it is naming what miel *does*, not enumerating a settings list.

     The specific claim being killed here is the one that was true for exactly
     one release: that promo extraction has no picker of its own. */
  test.each([
    ["README.md", readme],
    ["CONTRIBUTING.md", contributing],
  ])("%s does not tie the provider pick to a fixed set of tasks", (_name, doc) => {
    expect(doc).not.toMatch(/triage, repl(y|ies) and filter/i);
    expect(doc).not.toMatch(/all (three|four) tasks/i);
    expect(doc).not.toMatch(/no picker/i);
  });
...
// Issue #158. "What the AI sees" is the README's share of the disclosure the
// landing page and the privacy policy publish, and it said what those two said:
// that drafting a reply is the only request carrying a whole message body. A
// second body-bearing task (#156) made that false in three documents at once,
// so the list is core's and this section is checked against it — the values and
// the figure, not the wording, exactly as the OAuth walkthrough below.
describe("what the README publishes about the AI", () => {
  const seen = section(read("README.md"), "What the AI sees");

  test("is still a section", () => {
    expect(seen).toBeDefined();
  });

  test.each([...BODY_BEARING_TASKS])("names $task as a request that carries a body", (task) => {
    expect(seen!.toLowerCase()).toContain(task.label.toLowerCase());
  });

  test("quotes the truncation the prompts actually apply", () => {
    expect(seen).toContain(String(REPLY_BODY_TRUNCATION));
  });

  test("no longer says one of them is the only one", () => {
    expect(seen).not.toMatch(/only request that carries/i);
    expect(seen).not.toMatch(/the one place[^.]*body/i);
  });

  test("still says triage sends no body", () => {
    expect(seen).toMatch(/never the body|not the body/i);
  });

  // The section says where the figures live rather than being the third place
  // they are typed out.
  test("points at the module the figures come from", () => {
    expect(seen).toContain("packages/core/src/claudeUsage.ts");
  });
});
