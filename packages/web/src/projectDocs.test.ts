...
/**
 * The tokens miel defines for itself, in `index.css` rather than the vendored
 * sheet: the accent override and the three priority inks (#141). They are the
 * ones a reader is least likely to find by other means, since re-running the
 * registry add does not produce them.
 */
const ownTokens = unique(
  [...indexCss.matchAll(/--gousse-(?!shadow-)([a-z-]+)\s*:/g)].map((m) => m[1]!),
);
...
describe("the color tokens DESIGN.md documents", () => {
  ...
  // The same rule for the tokens that are ours: a `text-gousse-low-ink` nobody
  // documented is a colour the next restyling reinvents beside it.
  test.each(ownTokens)("`--gousse-%s`, defined in index.css, is in the §1 table too", (name) => {
    expect(tokenRow(name)).toBeDefined();
    expect(tokenRow(name)).toContain(`gousse-${name}`);
  });
  ...
  // #140. §1 legislates that the priority colors are used only for priority,
  // and now names the module that decides which one a `Priority` takes. A
  // pointer is only worth writing if it cannot rot, so the path is read out of
  // the doc and checked against the disk rather than restated here.
  test("the module §1 sends a recolouring at is a file that exists", () => {
    const named = /`(src\/lib\/[A-Za-z]+\.ts)`/.exec(designSection("1."))?.[1];
    expect(named).toBeDefined();
    expect(existsSync(join(packageRoot, named!))).toBe(true);
  });
