...
import { foldAccents as coreFoldAccents } from "@miel/core/accentFolding";
import { foldAccents } from "./confirmationKeywords";
...
/**
 * The folding this vocabulary is written against is core's own (#155), because
 * the promo prefilter in `@miel/core` matches its word lists the same way. An
 * identity check rather than a behaviour one: two copies that agree today would
 * pass every case above, and the point is that there are not two.
 */
describe("foldAccents", () => {
  it("is core's function, not a second copy of it", () => {
    expect(foldAccents).toBe(coreFoldAccents);
  });

  it("folds diacritics and expands ß, which is why the lists are ASCII", () => {
    expect(foldAccents("Vérifiez")).toBe("Verifiez");
    expect(foldAccents("Bestätigung")).toBe("Bestatigung");
