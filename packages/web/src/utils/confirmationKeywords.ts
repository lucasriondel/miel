 ...
 *
 * The implementation moved to `@miel/core/accentFolding` when core's promo
 * prefilter needed the same folding for the same reason (#155) — one answer to
 * "does `Ermäßigung` match `ermassigung`", not one per package. It is still
 * exported from here, since this is the module the vocabulary below is written
 * against.
 */
export { foldAccents } from "@miel/core/accentFolding";
