...
- what leaves the machine — the data each AI task sends the provider it runs on, documented in `packages/core/src/claudeUsage.ts`
...
The API is not meant to face the public internet. It authenticates every non-`/health` route with a single shared bearer token, CORS defaults to the web dev server's dev origins (`http://localhost:5230` and `https://miel.localhost`), and there is no per-user model. Put it behind your own network boundary. In production, set `TOKEN_ENCRYPTION_KEY` — refresh tokens are encrypted with it, and it is required there for that reason.
